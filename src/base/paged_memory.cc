...
PagedMemory PagedMemory::Allocate(size_t req_size, int flags) {
  ...
  PERFETTO_SOLACE_CHECK(rounded_up_size >= req_size);
  ...
  if (ptr == MAP_FAILED && (flags & kMayFail))
    return PagedMemory();
  PERFETTO_SOLACE_CHECK(ptr && ptr != MAP_FAILED);
  char* usable_region = reinterpret_cast<char*>(ptr) + GuardSize();
...
bool PagedMemory::AdviseDontNeed(void* p, size_t size) {
  PERFETTO_SOLACE_DCHECK(p_);
  PERFETTO_SOLACE_DCHECK(p >= p_);
  PERFETTO_SOLACE_DCHECK(static_cast<char*>(p) + size <= p_ + size_);
#if PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_OS_WIN) || PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_OS_NACL)
  // Discarding pages on Windows has more CPU cost than is justified for the
  // possible memory savings.
  return false;
#else   // PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_OS_WIN) ||
        // PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_OS_NACL)
...
void PagedMemory::EnsureCommitted(size_t committed_size) {
  PERFETTO_SOLACE_DCHECK(committed_size > 0u);
  PERFETTO_SOLACE_DCHECK(committed_size <= size_);
#if PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_OS_WIN)
  ...
  PERFETTO_SOLACE_DCHECK(num_additional_chunks * kCommitChunkSize >= delta);
