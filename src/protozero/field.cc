...
void Field::SerializeAndAppendToInternal(Container* dst) const {
  ...
  switch (type_) {
    ...
    default:
      PERFETTO_SOLACE_FATAL("Unknown field type %u", type_);
  }
  size_t written_size = static_cast<size_t>(wptr - start);
  PERFETTO_SOLACE_DCHECK(written_size > 0 && written_size < pu::kMaxMessageLength);
  PERFETTO_SOLACE_DCHECK(initial_size + written_size <= dst->size());
