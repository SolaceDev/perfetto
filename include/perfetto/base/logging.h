...
#ifndef INCLUDE_PERFETTO_SOLACE_BASE_LOGGING_H_
#define INCLUDE_PERFETTO_SOLACE_BASE_LOGGING_H_
...
#if PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_FORCE_DCHECK_ON)
#define PERFETTO_SOLACE_DCHECK_IS_ON() 1
#elif PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_FORCE_DCHECK_OFF)
#define PERFETTO_SOLACE_DCHECK_IS_ON() 0
#elif defined(DCHECK_ALWAYS_ON) ||                                         \
    (!defined(NDEBUG) && (PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_STANDALONE_BUILD) || \
                          PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_CHROMIUM_BUILD) ||   \
                          PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_ANDROID_BUILD)))
#define PERFETTO_SOLACE_DCHECK_IS_ON() 1
#else
#define PERFETTO_SOLACE_DCHECK_IS_ON() 0
#endif

#if PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_FORCE_DLOG_ON)
#define PERFETTO_SOLACE_DLOG_IS_ON() 1
#elif PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_FORCE_DLOG_OFF)
#define PERFETTO_SOLACE_DLOG_IS_ON() 0
#else
#define PERFETTO_SOLACE_DLOG_IS_ON() PERFETTO_SOLACE_DCHECK_IS_ON()
#endif

#if defined(PERFETTO_SOLACE_ANDROID_ASYNC_SAFE_LOG)
...
// Enable the "Print the most recent PERFETTO_SOLACE_LOG(s) before crashing" feature
// on Android in-tree builds and on standalone builds (mainly for testing).
// This is deliberately no PERFETTO_SOLACE_OS_ANDROID because we don't want this
...
#if defined(PERFETTO_SOLACE_ANDROID_ASYNC_SAFE_LOG)
#define PERFETTO_SOLACE_ENABLE_LOG_RING_BUFFER() 0
#elif PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_ANDROID_BUILD)
#define PERFETTO_SOLACE_ENABLE_LOG_RING_BUFFER() 1
#elif PERFETTO_SOLACE_BUILDFLAG(PERFETTO_SOLACE_STANDALONE_BUILD) && \
...
// threads.
PERFETTO_SOLACE_EXPORT void SetLogMessageCallback(LogMessageCallback callback);

PERFETTO_SOLACE_EXPORT void LogMessage(LogLev,
                                const char* fname,
                                int line,
                                const char* fmt,
                                ...) PERFETTO_SOLACE_PRINTF_FORMAT(4, 5);
...
// perfetto.gni.
PERFETTO_SOLACE_EXPORT void EnableStacktraceOnCrashForDebug();
...
PERFETTO_SOLACE_EXPORT void MaybeSerializeLastLogsForCrashReporting();
#else
...
#define PERFETTO_SOLACE_FATAL(fmt, ...)       \
  do {                                 \
    ...
    PERFETTO_SOLACE_IMMEDIATE_CRASH();        \
  } while (0)

#if defined(__GNUC__) || defined(__clang__)
#define PERFETTO_SOLACE_PLOG(x, ...) \
  PERFETTO_SOLACE_ELOG(x " (errno: %d, %s)", ##__VA_ARGS__, errno, strerror(errno))
#else
// MSVC expands __VA_ARGS__ in a different order. Give up, not worth it.
#define PERFETTO_SOLACE_PLOG PERFETTO_SOLACE_ELOG
#endif

#define PERFETTO_SOLACE_CHECK(x)                            \
  do {                                               \
    if (PERFETTO_SOLACE_UNLIKELY(!(x))) {                   \
      PERFETTO_SOLACE_PLOG("%s", "PERFETTO_SOLACE_CHECK(" #x ")"); \
      PERFETTO_SOLACE_IMMEDIATE_CRASH();                    \
    }                                                \
  } while (0)
...
#define PERFETTO_SOLACE_DPLOG(x, ...) \
  PERFETTO_SOLACE_DLOG(x " (errno: %d, %s)", ##__VA_ARGS__, errno, strerror(errno))
#else
// MSVC expands __VA_ARGS__ in a different order. Give up, not worth it.
#define PERFETTO_SOLACE_DPLOG PERFETTO_SOLACE_DLOG
#endif
...
#define PERFETTO_SOLACE_DCHECK(x) PERFETTO_SOLACE_CHECK(x)
#define PERFETTO_SOLACE_DFATAL(...) PERFETTO_SOLACE_FATAL(__VA_ARGS__)
#define PERFETTO_SOLACE_DFATAL_OR_ELOG(...) PERFETTO_SOLACE_DFATAL(__VA_ARGS__)

#else  // PERFETTO_SOLACE_DCHECK_IS_ON()

#define PERFETTO_SOLACE_DCHECK(x) \
  do {                     \
  } while (false && (x))
